...
export function KpiCard({ kpi }: { kpi: Kpi }) {
  ...
  const progressFill = toneClasses(kpi.progressTone ?? "blue").fill

  return (
    <div className="flex flex-col gap-1.5 rounded-[9px] border border-border bg-card p-3 shadow-[0_2px_8px_-2px_rgba(15,44,77,.10),0_1px_2px_rgba(15,44,77,.05)]">
      ...
      {typeof kpi.progress === "number" && (
        <div className="mt-0.5 h-[3px] w-full overflow-hidden rounded-full bg-muted">
          <div className={`h-full rounded-full ${progressFill}`} style={{ width: `${kpi.progress}%` }} />
        </div>
      )}
    </div>
  )
