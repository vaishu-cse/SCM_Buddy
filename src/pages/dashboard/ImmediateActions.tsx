    ...
    <div className="rounded-[10px] border border-border bg-white shadow-[0_2px_8px_-2px_rgba(15,44,77,.10),0_1px_2px_rgba(15,44,77,.05)]">
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-14 font-semibold text-foreground">Immediate actions</span>
          <span className="hidden font-12 text-muted-foreground sm:inline">urgency, then age</span>
        </div>
      ...
      {actions.length === 0 ? (
        <div className="flex flex-col items-center gap-1 px-4 py-10 text-center">
          ...
          <div className="font-12 font-medium text-foreground">Queue clear on {scope}</div>
        </div>
      ) : (
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-2.5">
        ...
        {sorted.length > 3 && (
          <button
            ...
            className="shrink-0 rounded-md border border-border px-2.5 py-1 font-12 font-medium text-foreground hover:bg-muted"
          >
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate font-12 font-medium text-foreground">{item.title}</div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        {item.flag && (
          <span
            className={`rounded-[5px] border px-1.5 py-0.5 font-10 font-semibold uppercase tracking-[0.03em] ${flagTone.bg} ${flagTone.border} ${flagTone.fg}`}
          >
            {item.flag}
          </span>
        )}
        <span className="font-12 font-semibold text-primary">{item.value}</span>
        <button
          onClick={() => onResolve(item)}
          className="whitespace-nowrap rounded-[7px] bg-primary px-2.5 py-1.5 font-12 font-semibold text-primary-foreground hover:bg-brand-hover"
        >
          {item.cta} →
        </button>
        <button
